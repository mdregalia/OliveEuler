...
Tree::Tree(){
	...
	ifstream myfile ("p067_triangle.txt");
	string line;
	int i = 0;
	...
	if (myfile){
		while (getline(myfile, line)){
			...
			while (getline (myline, token, ' ')){
				//cout<< token << "\n";
				buildarray[i] = atoi(token.c_str());
				//cout << buildarray[i] << " i: " << i <<"\n";
				i++;
			}
		}
	}
...
void Tree::setuptree(){ //builds tree from bottom up
	...
	int place = 5049; //last number in the array
	Node * currentnode;
	...
	for (int i = 99; i >= 0; i--){ //moves through each layer //13 should be zero
